...
class About extends Component<Props, State> {
  ...
  componentDidMount = async () => {
    const response = await networkService.allEvents();
      ...
      >
        <View style={{ paddingHorizontal: 20 }}>
          ...
          <RCViewFlex style={{ flexDirection: "column" }}>
            ...
            >
              {this.state.event ? this.state.event.name : "Kroscon"}
            </Text>
            <View style={{ flexDirection: "row" }}>
              <Text style={{ fontWeight: "bold" }}>Lokalizacja: </Text>
              <Text>Strzyżów</Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              <Text style={{ fontWeight: "bold" }}>Czas trwania: </Text>
          ...
          <RCViewFlex style={{ flexDirection: "column" }}>
            ...
            <Text>
              {!this.state.event
                ? ""
                : String(this.state.event.description).substring(
                    0,
            ...
            >
              <Text style={{ paddingHorizontal: 5 }}>
                {this.state.subString === undefined ? "Zwiń" : "Rozwiń"}
              </Text>
            </TouchableOpacity>
          </RCViewFlex>
        </View>
...
const SContainer = styled(ScreenContainer).attrs(props => ({
  styleContent: { flex: 1, backgroundColor: props.theme.colors.primaryColor }
}));
